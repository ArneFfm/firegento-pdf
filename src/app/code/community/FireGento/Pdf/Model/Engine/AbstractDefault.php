 ...
 */
abstract class FireGento_Pdf_Model_Engine_AbstractDefault extends Mage_Sales_Model_Order_Pdf_Abstract
{
    ...
    protected function insertLogo(&$page, $store = null)
    {
        ...
        if ($image) {
            ...
            if (is_file($image)) {
                ...
                $top         = 820; //top border of the page - 10px for Heading
     ...
     * @param null $store
     */
    protected function insertTitle(&$page, $store = null)
    {
        ...
        $top = 815;
        $title =  trim(strip_tags(Mage::getStoreConfig('sales_pdf/invoice/title')));
        $page->drawText($title,
            $this->getAlignCenter($title, 80, 440, $font, 14),
            $top,
            'UTF-8');
        $top -= 10;
    ...
    protected function insertAddress(&$page, $store = null)
    {
        ...
        $tel = Mage::getStoreConfig('general/imprint/telephone');
        $page->drawText(trim(strip_tags($tel)),
            $x1,
            $top,
            'UTF-8');
        $top -= 10;
        $ust = Mage::getStoreConfig('general/imprint/vat_id');
        $page->drawText(trim(strip_tags($ust)),
            $x1,
            $top,
            'UTF-8');
        $top -= 10;
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $page->drawText(
            Mage::helper('firegento_pdf')->__('Order Date: ') . Mage::helper('core')->formatDate(
        ...
        $page->drawText(
            Mage::helper('firegento_pdf')->__('Performance date: ') . Mage::helper('core')->formatDate(
                $order->getCreatedAtStoreDate(), 'medium', false
            ),
            35,
            ($top -= 15),
    ...
    protected function customTransalation($value) {
        if (Mage::helper('firegento_pdf')->__($value)) {
            return Mage::helper('firegento_pdf')->__($value);
        }
        return false;
    }
